...
class GridFileHelper:
	...
	def parseYaml(self, yamlContent: dict, gridFile: str):
		...
		try:
			self.randomize = self.gridObj.get("randomize")
			if self.randomize is not None:
				self.randomize = self.randomize.lower() == 'true'
		except:
			self.randomize = False

		try:
			self.sort = self.gridObj.get("sort")
			if self.sort is not None:
				self.sort = self.sort.lower() != 'false'
			else:
				self.sort = True
		except:
			self.sort = False
...
class GridRunner:
	...
	def calculateSteps(self, processors):
		for item in processors:
			...
			steps += max(DataLog(getAll(item.giga['multistep']), False, 2))
			if hasattr(item, "hr_second_pass_steps"):
				steps += item.hr_second_pass_steps
			self.totalSteps += steps

	def sortProcs(self, proceslist):
		startTime = datetime.datetime.now()
		def keyFunction(obj):
			sort_group = obj.giga.get('sortgroup', '')
			model = obj.override_settings.get('sd_model_checkpoint')
			vae = obj.override_settings.get('sd_vae')
			sampler = getattr(obj,'sampler_name')
			matches = re.findall(r'<(.*?)>', obj.prompt)
			bracket_content = str(tuple(HTMLModule.escape(match) for match in matches)) if matches else ''

			
			return (sort_group, model, vae, sampler, bracket_content)
		
		a = sorted(proceslist, key=keyFunction)
		DataLog(f"sorting took: {quickTimeMath(startTime=startTime)}", True, 2)
		return a
	
	def get_attribute_tuple(self, obj):
		"""Generate a tuple of all attribute values, converting mutables to immutables."""
		def convert_to_immutable(value):
			"""Convert mutables to immutables."""
			return str(value)
		
		return tuple(convert_to_immutable(getattr(obj, attr)) for attr in dir(obj) if not attr.startswith('__') and hasattr(obj, attr))
	...
	def run(self, dry: bool):
		...
		promptBatchList = []
		if not dry:
			self.attributeTuples = set()
			self.processList = []
			if grid.randomize: random.shuffle(self.valueSets)
			for vSet in self.valueSets:
				if vSet.skip:
					continue
				iteration += 1
				#if not dry:
				DataLog(f'[mainRun] On {iteration}/{self.totalRun} ... Set: {vSet.data}, file {vSet.filepath}', False, 1) #excessive logging causes slowdowns
			...
			if grid.sort == True:
				promptBatchList = self.sortProcs(promptBatchList)
			...
			for _, p2 in enumerate(promptBatchList):
				...
				print(f"steps remaining: {stepsremaining} / {self.totalSteps}")
	...
	def comparePrompt(self, processor1, processor2):
		# Regular expression pattern to extract content within triangle brackets
		pattern = r'<(.*?)>'
		
		# Extracting content within triangle brackets from both prompts
		matches1 = re.findall(pattern, processor1.prompt)
		matches2 = re.findall(pattern, processor2.prompt)
		
		# Comparing the number of occurrences of content within triangle brackets
		if len(matches1) != len(matches2):
			return False
		
		# Comparing the content within triangle brackets
		for content1, content2 in zip(matches1, matches2):
			if content1 != content2:
				return False
		
		# If both prompts have no content within triangle brackets, return True
	...
	def batchPromptsGrouping(self, Processors: list[StableDiffusionProcessing]) -> list:
		...
		for i, processor1 in enumerate(Processors):
			if processor1 == Processor2:
				...
				continue
			if (not self.compareProcessor(processor1, Processor2, ['prompt', 'all_prompts', 'all_negative_prompts', 'negative_prompt', 'seed', 'subseed', 'gigauncomp'], False) \
			or processor1.batch_size - 1 == len(ProcessorGroup) - 1) \
			and self.comparePrompt(processor1,Processor2):
