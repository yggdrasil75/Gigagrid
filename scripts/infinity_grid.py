...
def applyModel(p, v):
	opts.sd_model_checkpoint = getBestInList(v, map(lambda m: m.title, sd_models.checkpoints_list.values()))
	sd_models.reload_model_weights()

def cleanModel(p, v):
	actualModel = getBestInList(v, map(lambda m: m.title, sd_models.checkpoints_list.values()))
...
#def getModelFor(name:str):
#	global cplist
#	if not cplist:
#		print(sd_models.checkpoints_list.values())
#		cplist = [str(x).lower().replace(' ', '').replace('[', '').replace(']', '').strip() for x in sd_models.checkpoints_list.values()]
#		#cplist = map(str.lower().replace(' ', '').replace('[', '').replace(']', '').strip(), sd_models.checkpoints_list.values())
#	elif len(cplist) < 1:
#		print(sd_models.checkpoints_list.values())
#		cplist = [str(x).lower().replace(' ', '').replace('[', '').replace(']', '').strip() for x in sd_models.checkpoints_list.values()]
#	if str(name).lower().replace(' ', '').replace('[', '').replace(']', '').strip() in cplist:
...
def tryInit():
	...
	registerMode("Styles", GridSettingMode(dry=True, type="text", apply=applyStyles, valid_list=lambda: list(shared.prompt_styles.styles)))
	...
	try:
		...
		if len(scriptList) == 1:
			...
			registerMode("[DynamicThreshold] Experiment Mode", GridSettingMode(dry=True, type="decimal", min=0, max=1000000, apply=applyField("dynthres_experiment_mode")))
			registerMode("[DynamicThreshold] scheduler Value", GridSettingMode(dry=True, type="decimal", min=0, max=100, apply=applyField("dynthres_scheduler_val")))
			registerMode("[DynamicThreshold] Scaling Startpoint", GridSettingMode(dry=True, type="text", apply=applyField("dynthres_scaling_startpoint"), valid_list=lambda: list(['ZERO', 'MEAN'])))
			registerMode("[DynamicThreshold] Variability Measure", GridSettingMode(dry=True, type="text", apply=applyField("dynthres_variability_measure"), valid_list=lambda: list(['STD', 'AD'])))
			registerMode("[DynamicThreshold] Interpolate Phi", GridSettingMode(dry=True, type="decimal", min=0, max=1, apply=applyField("dynthres_interpolate_phi")))
			registerMode("[DynamicThreshold] Separate Feature Channels", GridSettingMode(dry=True, type="boolean", apply=applyField("dynthres_separate_feature_channels")))
...
def a1111GridCallParamAddHook(gridCall: core.SingleGridCall, p: str, v: str) -> bool:
	if gridCall.grid.minWidth is None:
		gridCall.grid.minWidth = gridCall.grid.initialPromptskey.width
	if gridCall.grid.minHeight is None:
		gridCall.grid.minHeight = gridCall.grid.initialPromptskey.height
	...
	if tempstring.startswith('promptreplace'):
		...
		gridCall.grid.minWidth = min(gridCall.grid.minWidth, v)
		return True
	elif tempstring in ['height', 'outheight']:
		gridCall.grid.minHeight = min(gridCall.grid.minHeight, v)
		return True
	return False
